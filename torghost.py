...
def show_status():
        print t()+" Showing current status of TorGhost..."
        print t()+" #1 Verified IP: "+bcolors.GREEN+ip()+bcolors.ENDC
        print t()+" #2 Verified IP: "+bcolors.GREEN+os.popen('curl ipecho.net/plain -s').read()+bcolors.ENDC
        print t()+" Your host addresses for comparition:\n\n"+bcolors.GREEN+os.popen("ip addr show | grep 'inet ' | awk '{print $2}' | cut -f1 -d'/'").read()+bcolors.ENDC+"\n"
